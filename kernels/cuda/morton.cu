...
//   const auto i = static_cast<uint32_t>((xyz.x - min_coord) / range *
//   bit_scale); const auto j = static_cast<uint32_t>((xyz.y - min_coord) /
//   range * bit_scale); const auto k = static_cast<uint32_t>((xyz.z -
//   min_coord) / range * bit_scale); return m3D_e_magicbits(i, j, k);
// }
...
__global__ void k_ComputeMorton_Kernel(const glm::vec4* d_xyz,
  ...
  for (auto i = idx; i < n; i += stride) {
    morton_keys[i] = gpu::xyz_to_morton32(d_xyz[i], min_coord, range);
  }
}
